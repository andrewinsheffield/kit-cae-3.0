...
#include <omniCaeHdf5/hdf5FieldArray.h>
...
{
    return (prim.IsValid() && prim.IsA<pxr::OmniCaeHdf5Hdf5FieldArray>());
}
...
carb::ObjectPtr<IFieldArray> DataDelegate::getFieldArray(pxr::UsdPrim prim, pxr::UsdTimeCode time)
{
    ...
    const pxr::OmniCaeHdf5Hdf5FieldArray hdf5Array(prim);
    std::string hdf5Path;
    ...
    try
    {
        ...
        if (settings->getAsBool("/exts/omni.cae.delegate.hdf5/convertDoubleToFloats"))
        {
