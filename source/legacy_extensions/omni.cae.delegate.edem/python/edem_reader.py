...
from omni.cae.core import progress
            ...
            for node_name, node in h0["/CreatorData/0/ParticleTypes"].items():
                ...
                if spheres_node := node.get("spheres"):
                    ...
                    for idx, name in enumerate(names):
                        position = tuple(float(x) for x in pos[idx])
                        physical_radius = float(physicalRadius[idx])
                        contact_radius = float(contactRadius[idx])
                        sphere = UsdGeom.Sphere.Define(stage, xform.GetPath().AppendChild(Tf.MakeValidIdentifier(name)))
                        sphere.CreateRadiusAttr(physical_radius)
