...
{
    return (prim.IsValid() && prim.IsA<pxr::OmniCaeCgnsCgnsFieldArray>());
}
...
carb::ObjectPtr<IFieldArray> DataDelegate::getFieldArray(pxr::UsdPrim prim, pxr::UsdTimeCode time)
{
    ...
    const pxr::OmniCaeCgnsCgnsFieldArray cgnsArray(prim);
    ...
    if (settings->getAsBool("/exts/omni.cae.delegate.cgns/convertDoubleToFloats"))
    {
    ...
    if (etype == ElementType::int64)
    {
        ...
        if (idx != std::string::npos && fieldPath.substr(idx + 1) == "ElementConnectivity" &&
            settings->getAsBool("/exts/omni.cae.delegate.cgns/convertElementConnectivityToInt32"))
        {
